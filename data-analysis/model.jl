...
function reset_model(p)
    ...
    for m in sample(1:p.M,p.λ,replace=false)
        Y[m] = gen_af(m, X[1], p.N, p.filaments, p.P, p.L,p.δ)
    end
...
function gen_af(m,x,N,filaments,P,L,δ)
    all_af = [n for n in filaments if mf_overlap(x[N+m],x[n],L)]
    sort!(all_af)
    if length(all_af) ≥ 2
        af = sample(all_af,2,replace=false)
        K1, R1 = ma_interval(x[N+m],x[af[1]],P[af[1]],L,δ)
        K2, R2 = ma_interval(x[N+m],x[af[2]],P[af[2]],L,δ)
        if ((P[af[1]] != P[af[2]]) && !((K1 < 0) || (R1 > 0)) && !((K2 < 0) || (R2> 0))) && (abs(((x[af[1]] + x[af[2]])/2 - x[N+m])) < 0.05)
...
function update_af(x,y,params)
    ...
    for m in motors_to_attach
        y[m] = gen_af(m,x,params.N,params.filaments,params.P,params.L,params.δ)
    end
